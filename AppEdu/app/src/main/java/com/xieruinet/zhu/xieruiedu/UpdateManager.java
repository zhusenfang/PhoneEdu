...
public class UpdateManager
{
	 ...
	 * 检测软件更新
	 */
	public void checkUpdate(String versionname,String serverip)
	{
		if (isUpdate(versionname,serverip ))
		{
			// 显示提示对话框
			showNoticeDialog();
		} else
	 ...
	 * @return
	 */
	private boolean isUpdate(String versionname,String serverip )
	{
		try {
			//从资源文件获取服务器 地址
			//包装成url的对象
			URL url = new URL(serverip + "/vessioninfo/appupdate.xml");
			...
			if(serversoin.equals(versionname)){
				...
				mHashMap =new HashMap<String, String>();// service.parseXml(inStream);
				mHashMap.put("version",info.getVersion());
				mHashMap.put("name","apptempflie");
				mHashMap.put("url",info.getUrl());
				return true;
			}
